...
object Interpreter {
  ...
  def interpretTerm(context: Context, tm: Term): Term = tm match {
    ...
    case Pair(first,second) =>
      // AM define the terms and return the new term
      val fI = interpretTerm(context, first)
      val sI = interpretTerm(context, second)

      Pair(fI, sI)

    case Projection(x) =>
      // AM define the term and return the new projection
      val xI = interpretTerm(context, x)
...
object Closer {
  ...
  def closeTerm(context: Context, tm: Term): Term = tm match {
    ...
    case Pair(first, second) =>
      // AM Define the terms and close the pair
      val fC = closeTerm(context, first)
      val sC = closeTerm(context, second)

      Pair(fC, sC)

    case Projection(x) =>
      // AM Define the term and close the projection
