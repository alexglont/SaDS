...
object Printer {
   ...
   def printType(t: Type): String = t match {
     ...
     // AM Printing ProdType
     case ProductType(x, y) => "(" + printType(x) + "*" + printType(y) + ")"
   }
   ...
   def printTerm(t: Term): String = t match {
     ...
     // AM Printing pairs
     case Pair(first, second) => "(" + printTerm(first) + "," + printTerm(second) + ")"

     // AM Printing projections
     case Projection(x) => printTerm(x)
