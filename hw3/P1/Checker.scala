...
object Checker {
  ...
  // context |- tp : type
  def checkType(context: Context, tp: Type){
    tp match {
      case TypeRef(n) =>
      ...
      case FunType(f,t) =>
        ...
        //chck Product asguments
      case ProductType(x,y) =>
        checkType(context, x)
        checkType(context, y)
    }
  }
  ...
  def inferType(context: Context, tm: Term): Type = {
    tm match {
      ...
      case Pair(first, second) =>
        // AM Checks both types and returns a product
        val fType = inferType(context, first)
        val sType = inferType(context, second)
        ProdType(fType, sType)

      case Projection(x) =>
        // AM Check the product type and return it
        val xType = inferType(context, x)
        xType
    }
  }
}
